...
resolve(url)})}).then(getData).then(res=>{var data=JSON.parse(res);convertData(data.body.works).then(data=>{bookmarkData=(()=>{if(data.bookmark)return data.bookmark;return[]})();userData=(()=>{if(data.user)return data.user;return{}})();var optionTextarea=document.getElementById('optionTextarea');optionTextarea.value='完了'})})}

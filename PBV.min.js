...
else{var imgItem=iframeDoc.querySelectorAll('div[role]');for(var i=0;imgItem[i];i++){try{var a=imgItem[i].parentElement.parentElement.parentElement.parentElement.parentElement.getElementsByTagName('a');var id=a[2].href.match(/(\w+)$/)[1];userData[id]={"name":a[3].textContent,"icon":window.getComputedStyle(imgItem[i],null).getPropertyValue('background-image').match(/\("(.+?)"\)/)[1]}}
catch(e){console.error(e)}}}

...
else{reading=!1;var overlay=document.querySelector('#pbvContainer .overlay');overlay.classList.add('none');var but=document.getElementById('bookmarkReadStrat');but.textContent='開始';clearInterval(iframeTimer);iframeTimer=setInterval(()=>{changeBookmarkAddPageCount()},1000)}},!1);element=document.getElementById('butDataShow');element.addEventListener('click',()=>{showBookmarkData()},!1);element=document.getElementById('butDataOverwrite');element.addEventListener('click',()=>{overwriteBookmarkData()},!1);element=document.getElementById('butDownloadData');element.addEventListener('click',()=>{downloadData()},!1);element=document.getElementById('butLoadWithAjax');element.addEventListener('click',()=>{loadDataWithAjax()},!1);element=document.getElementById('butDataUpload');element.addEventListener('click',()=>{document.getElementById('dataFile').click()},!1);element=document.getElementById('dataFile');element.addEventListener('change',uploadFile,!1)}};function loadToLocalStorage(){return new Promise(function(resolve,reject){var data=JSON.parse(localStorage.getItem('pbv'));if(!isObject(data))data={};bookmarkData=(()=>{if(data.bookmark)return data.bookmark;return[]})();userData=(()=>{if(data.user)return data.user;return{}})();resolve()})}
...
getData(url).then(res=>{new Promise(function(resolve,reject){var data=JSON.parse(res);localStorage.setItem('pbv',JSON.stringify(data));resolve()}).then(loadToLocalStorage()).then(saveToLocalStorage()).then(()=>{optionTextarea.value='完了'}).catch(err=>{optionTextarea.value='データが無効です。';console.error(err)})},err=>{optionTextarea.value='URLが無効です。';console.error(err)})}
function uploadFile(e){var optionTextarea=document.getElementById('optionTextarea');var file=e.target.files;var reader=new FileReader();reader.readAsText(file[0]);reader.onload=()=>{new Promise(function(resolve,reject){var data=JSON.parse(reader.result);localStorage.setItem('pbv',JSON.stringify(data));resolve()}).then(loadToLocalStorage()).then(saveToLocalStorage()).then(()=>{optionTextarea.value='完了'}).catch(err=>{optionTextarea.value='データが無効です。';console.error(err)})}}})()

...
else{reading=!1;var overlay=document.querySelector('#pbvContainer .overlay');overlay.classList.add('none');var but=document.getElementById('bookmarkReadStrat');but.textContent='開始';clearInterval(iframeTimer);iframeTimer=setInterval(()=>{changeBookmarkAddPageCount()},1000)}},!1);element=document.getElementById('butDataShow');element.addEventListener('click',()=>{showBookmarkData()},!1);element=document.getElementById('butDataOverwrite');element.addEventListener('click',()=>{overwriteBookmarkData()},!1);element=document.getElementById('butLoadWithAjax');element.addEventListener('click',()=>{loadDataWithAjax()},!1)}};function loadToLocalStorage(){return new Promise(function(resolve,reject){var data='';var pbvCount=+localStorage.getItem('pbvCount');for(var i=0;i<pbvCount;i++){var key='pbv_'+i;data+=localStorage.getItem(key)}
data=JSON.parse(data);if(!isObject(data))data={};bookmarkData=(()=>{if(data.bookmark)return data.bookmark;return[]})();userData=(()=>{if(data.user)return data.user;return{}})();resolve()})}
function saveToLocalStorage(){return new Promise(function(resolve){var data={"bookmark":bookmarkData,"user":userData};data=JSON.stringify(data);createDataText(data);resolve()})}
function clearLocalStorage(){return new Promise(function(resolve){var pbvCount=+localStorage.getItem('pbvCount');for(var i=0;i<pbvCount;i++){var key='pbv_'+i;localStorage.removeItem(key)}
resolve()})}
function createDataText(data){return new Promise(function(resolve){clearLocalStorage().then(()=>{var maxLength=1000000;var pbvCount=Math.ceil(data.length/maxLength);localStorage.setItem('pbvCount',pbvCount);for(var i=0;i<pbvCount;i++){var key='pbv_'+i;var value=data.substr(i*maxLength,maxLength);localStorage.setItem(key,value)}});resolve()})}
...
getData(url).then(res=>{new Promise(function(resolve,reject){createDataText(res);resolve()}).then(()=>{optionTextarea.value='完了'}).catch(err=>{optionTextarea.value='データが無効です。';console.error(err)})},err=>{optionTextarea.value='URLが無効です。';console.error(err)}).then(loadToLocalStorage())}})()

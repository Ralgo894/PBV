(()=>{var NEW_PAGE_LI_QUERY='#root ul.xq6AsQu.KvF6Ntf li';var pc=(()=>{if(document.getElementById('root'))return!0;return!1})();var documentElement=(()=>{if(navigator.userAgent.toLowerCase().match(/webkit|msie 5/)){if(navigator.userAgent.indexOf('Chrome')!=-1){return document.documentElement}
...
else{var addStyle=document.createElement('style');var rule=document.createTextNode(pbvStyle);addStyle.media='screen';addStyle.type='text/css';addStyle.appendChild(rule);document.head.appendChild(addStyle);container=document.querySelector('#contents>div>div:nth-child(2)>div:nth-child(6)');container.id='pbvContainer';container.innerHTML=pbvBookmarkHTML}
...
resolve()})}
createRandomData();function createSearchData(){return new Promise(function(resolve,reject){searchData=[];var searchBox=document.getElementById('inputSearch').value;if(searchBox==''){bookmarkViewMode=(()=>{if(document.getElementById('radioRandom').checked)return'random';return'normal'})();resolve();return}
var arr=[];if(document.getElementById('radioRandom').checked){arr=randomData}
else{arr=bookmarkData}
var searchTexts=searchBox.replace(/　/g,' ').split(' ');var condition=[[]];for(var i=0;i<searchTexts.length;i++){if(searchTexts[i]!='AND'&&searchTexts[i]!='OR'&&searchTexts[i]!='NOT'){if(searchTexts[i-1]=='OR'){condition.push([searchTexts[i]])}
...
var compile=function(cond){var joinAnd=function(arr){return'^(?=[\\s\\S]*'+arr.join(')(?=[\\s\\S]*')+')'};var joinOr=function(arr){return'(?:'+arr.join('|')+')'};var escape=function(str){return str.replace(/(?=[(){}\[\].*\\^$?])/,'\\')};var rx=joinOr(cond.map(function(inner){return joinAnd(inner.map(escape))}));rx=rx.replace(/=\[\\s\\S\]\*-/g,'![\\s\\S]*');return new RegExp(rx)};var matcher=compile(condition);for(var i=0;i<arr.length;i++){var testStr=arr[i].title+' '+arr[i].user+' '+arr[i].tags.join();if(matcher.test(testStr)){searchData.push(arr[i])}}
...
function readBookmarkPageData(){var p=changeBookmarkAddPageCount();if(!p)return;var iframe=document.getElementById('iframeBookmark');var iframeDoc=iframe.contentWindow.document;if(bookmarkAddPage=='old'){var imgItem=iframeDoc.querySelectorAll('.js-legacy-mark-unmark-list .image-item');for(var i=imgItem.length-1;i>=0;i--){try{var imgElement=imgItem[i].getElementsByClassName('ui-scroll-view')[0];var data={};data.id=imgElement.getAttribute('data-id');data.title=imgItem[i].getElementsByClassName('title')[0].getAttribute('title');data.user=imgItem[i].getElementsByClassName('user')[0].getAttribute('data-user_name');data.userId=imgItem[i].getElementsByClassName('user')[0].getAttribute('data-user_id');data.count=(()=>{if(imgItem[i].querySelector('.page-count span'))return imgItem[i].querySelector('.page-count span').textContent;return'0'})();data.imageUrl=imgElement.getAttribute('data-src').replace(/^.+?img\/(.+?)master(.+?)$/,'$1square$2');data.userIconUrl='';data.ugoku=imgItem[i].getElementsByClassName('work')[0].classList.contains('ugoku-illust');data.tags=imgElement.getAttribute('data-tags').split(' ');addData.unshift(data);if(!userData[data.userId])userData[data.userId]={"icon":""};userData[data.userId].name=data.user}
catch(e){}}}
else{var imgItem=iframeDoc.querySelectorAll(NEW_PAGE_LI_QUERY);for(var i=imgItem.length-1;i>=0;i--){try{var a=imgItem[i].getElementsByTagName('a');var id=a[2].href.match(/(\w+)$/)[1];userData[id]={"name":a[3].textContent,"icon":a[2].style.backgroundImage.match(/"(.+)"/)[1]}}
catch(e){}}}
if(p>1){p--;var url=(()=>{if(bookmarkAddPage=='old')return'https://www.pixiv.net/bookmark.php?rest=show&p='+p;if(bookmarkAddPage=='new')return'https://www.pixiv.net/bookmark.php?id=9791957&rest=show&p='+p;return''})();iframe.src=url}
else{new Promise(resolve=>{if(bookmarkAddPage=='old'){if(addMode=='add'){addBookmarkData()}
else{bookmarkData=addData}}
resolve()}).then(()=>{return new Promise(resolve=>{setUserData();resolve()})}).then(()=>{saveToLocalStorage();var but=document.getElementById('bookmarkReadStrat');but.click()})}}
...
function setUserData(){if(bookmarkData==[])return;for(key in userData){bookmarkData.forEach((e)=>{if(e.userId==key){if(e.user!=userData[key].name)e.user=userData[key].name;if(e.userIconUrl!=userData[key].icon)e.userIconUrl=userData[key].icon}})}}
function showBookmarkData(){var confirm=window.confirm('データを表示します。');if(!confirm)return;var optionTextarea=document.getElementById('optionTextarea');optionTextarea.value=localStorage.getItem('pbv')}
function overwriteBookmarkData(){var confirm=window.confirm('データを上書きします。');if(!confirm)return;var optionTextarea=document.getElementById('optionTextarea');if(optionTextarea.value==''){optionTextarea.value='データが無効です。';return}
var data;try{new Promise(function(resolve,reject){data=JSON.parse(optionTextarea.value);bookmarkData=data.bookmark;userData=data.user;resolve()}).then(saveToLocalStorage()).then(()=>{optionTextarea.value='完了'})}
catch(e){optionTextarea.value='データが無効です。'}}
...
getData(url).then(res=>{try{new Promise(function(resolve,reject){var data=JSON.parse(res);bookmarkData=data.bookmark;userData=data.user;resolve()}).then(saveToLocalStorage()).then(()=>{optionTextarea.value='完了'})}
catch(e){optionTextarea.value='データが無効です。'}},err=>{optionTextarea.value='URLが無効です。';console.error(err)})}})()

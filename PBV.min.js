...
function readBookmarkPageData(){var p=changeBookmarkAddPageCount();if(!p)return;var iframe=document.getElementById('iframeBookmark');var iframeDoc=iframe.contentWindow.document;if(bookmarkAddPage=='old'){var imgItem=iframeDoc.querySelectorAll('.js-legacy-mark-unmark-list .image-item');for(var i=imgItem.length-1;i>=0;i--){try{var imgElement=imgItem[i].getElementsByClassName('ui-scroll-view')[0];var data={};data.id=imgElement.getAttribute('data-id');data.title=imgItem[i].getElementsByClassName('title')[0].getAttribute('title');data.user=imgItem[i].getElementsByClassName('user')[0].getAttribute('data-user_name');data.userId=imgItem[i].getElementsByClassName('user')[0].getAttribute('data-user_id');data.count=(()=>{if(imgItem[i].querySelector('.page-count span'))return imgItem[i].querySelector('.page-count span').textContent;return'1'})();data.imageUrl=imgElement.getAttribute('data-src').replace(/^.+?img\/(.+?)master(.+?)$/,'$1square$2');data.userIconUrl='';data.ugoku=imgItem[i].getElementsByClassName('work')[0].classList.contains('ugoku-illust');data.tags=imgElement.getAttribute('data-tags').split(' ');addData.unshift(data);if(userData[data.userId]==null){userData[data.userId]={"name":data.user,"icon":""}}

...
else{reading=!1;var overlay=document.querySelector('#pbvContainer .overlay');overlay.classList.add('none');var but=document.getElementById('bookmarkReadStrat');but.textContent='開始';clearInterval(iframeTimer);iframeTimer=setInterval(()=>{changeBookmarkAddPageCount()},1000)}},!1);element=document.getElementById('butDataShow');element.addEventListener('click',()=>{showBookmarkData()},!1);element=document.getElementById('butDataSave');element.addEventListener('click',()=>{overwriteBookmarkData()},!1);element=document.getElementById('butLoadWithAjax');element.addEventListener('click',()=>{loadDataWithAjax()},!1);element=document.getElementById('butGetDataWithApi');element.addEventListener('click',()=>{getDataWithApi()},!1)}};function loadToLocalStorage(){return new Promise(function(resolve,reject){dbStorage.get('pbv',(d)=>{var data=JSON.parse(d);if(!isObject(data))data={};bookmarkData=(()=>{if(data.bookmark)return data.bookmark;return[]})();userData=(()=>{if(data.user)return data.user;return{}})();resolve()})})}

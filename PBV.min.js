...
function saveToLocalStorage(dataStr){return new Promise(function(resolve){if(dataStr){localStorage.setItem('pbv',dataStr)}
else{var data={"bookmark":bookmarkData,"user":userData};localStorage.setItem('pbv',JSON.stringify(data))}
resolve()})}
...
resetHTML();setEvent();loadToLocalStorage();function addItem(){try{for(var i=0;i<viewCount;i++){var index=i+(loadCount*viewCount);var data=(()=>{if(bookmarkViewMode=='normal')return bookmarkData[index];if(bookmarkViewMode=='random')return randomData[index];return searchData[index]})();createItem(data)}}
...
else{optionTextarea.value='ループ終了';resolve_0(allData)}})};loop()})}

...
function getDataWithApi(){var confirm=window.confirm('データを取得しますか。');if(!confirm)return;var myId=location.search.match(/id=(\w+)/)[1];getData('https://www.pixiv.net/ajax/user/'+myId+'/illusts/bookmarks?tag=&offset=0&limit=1&rest=show').then(res=>{return new Promise(resolve=>{var data=JSON.parse(res);var total=data.body.total;var url='https://www.pixiv.net/ajax/user/'+myId+'/illusts/bookmarks?tag=&offset=0&limit='+total+'&rest=show'
resolve(url)})}).then(getData).then(res=>{var data=JSON.parse(res);convertData(data.body.works).then(data=>{if(window.confirm('データを上書きしますか。')){bookmarkData=(()=>{if(data.bookmark)return data.bookmark;return[]})();userData=(()=>{if(data.user)return data.user;return{}})()}
